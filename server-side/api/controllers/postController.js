...
exports.create_new_group = function(request, result){
	...
	specificAuth = true
	if (!common.perform_query(attributes, placeholders, skeleton, specificAuth, request, result)) {
		common.perform_query(attributes, placeholders, "SELECT id FROM groups WHERE group_name=groupName;", specificAuth, request, result)
	}
};
...
exports.create_new_user = function(request, result) {
	...
	if (!common.perform_query(attributes, placeholders, skeleton, specificAuth, request, result)) {
		common.perform_query(["_email"], ["_email"], "SELECT id from user_accounts WHERE email=_email;", specificAuth, request, result)
	}
};
...
exports.create_group_debt = function(request, result) {
	...
	specificAuth = true
	if (!common.perform_query(attributes, placeholders, skeleton, specificAuth, request, result)) {
		result.status(200).send(' ') // anything that's not an error should be interpreted as good 
	}
};
